class xhr {
  ...
  get(url, fun, info = null) {
    this.http.onreadystatechange = function() {
      if (this.readyState == 4)
        ...
        if (info == null) fun(0, "Connection Issue");
        else fun(0, "Connection Issue", info);
    };
  ...
  post(url, data, fun, info = null) {
    ...
    this.http.open("post", "/api/admin/notifications/", true);
...
class adminClass {
  ...
  submitNotifForm() {
    ...
    if ($('input[name=batchForNotif]:checked').val() == "new" || $('input[name=batchForNotif]:checked').val() == "imp" || $('input[name=batchForNotif]:checked').val() == "na") {
    ...
    this.http.post("/api/admin/notifications/", data, admin.listenSubmitNotifForm);
    document.getElementById("notifSendBtn").disabled = true;
  }
  listenSubmitNotifForm(status, res) {
    document.getElementById("notifSendBtn").disabled = false;
    try {
      ...
      document.getElementById("infoNotifManage").innerHTML = html;
      return false;
    }
    if (status == 200) {
      if (res.code == 200) {
