class result {
    ...
    getCourses(fun, ext) {
        var data = new FormData();
        data.append("request", "getCourses");
        this.http.post("/api/results/", data, fun, ext);
    }
    getResult() {
        ...
        try {
          ...
          if (status == 200) {
            if (res.code == 200) {
              ...
              console.log(res);
              var html = "<option id='"+ext[0]+" value=''>" + res.message + "</option>";
            }
          } else if (status == 0) {
            var html =
              "<option value='' id='"+ext[0]+" class='text-danger'>Failed to connect with server, check internet connection</option>";
          } else if (status == 403) {
            var html =
            "<option value='' id='"+ext[0]+" class='text-danger'>Access Denied, Login First</option>";

          } else {
            console.log(res);
            var html =
            "<option value='' id='"+ext[0]+" class='text-danger'>Something wrong</option>";            
          }
          document.getElementById(ext[1]).innerHTML = html;
    }
    getCoursesInSelect(status, res, ext) {
      try {
          var res = JSON.parse(res);
        } catch {
          console.log(status, res);
        ...
        if (status == 200) {
          if (res.code == 200) {
              ...
              for(var i = 0; i < res.data.length; i++) {
                  if(j==0) {
                      html = "<option id='"+ext[0]+"-def' value=''>Select Course</option>";
                      
                  }
                  course.courses[res.data[i].course_id] = res.data[i];
                  html+="<option id='"+ext[0]+"-"+res.data[i].course_id+"' value='"+res.data[i].course_id+"'>"+res.data[i].course_name+"</option>";
                  j++;
              }
              
          } else if (res.code == 201) {
            var html =
            '<option id="'+ext[0]+'-def" value="">No Course found</option>';
          } else if(res.code == 400) {
            var html = "<option id='"+ext[0]+" value=''>Bad request, please contact administrator with message - "+res.message+"</option>";
          } else {
            console.log(res);
            var html = "<option id='"+ext[0]+" value=''>" + res.message + "</option>";
          }
